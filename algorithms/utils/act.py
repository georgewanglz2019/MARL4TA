from .distributions import Bernoulli, Categorical, DiagGaussian, Dirichlet
import torch
...
class ACTLayer(nn.Module):
    ...
    def __init__(self, action_space, inputs_dim, use_orthogonal, gain):
        ...
        if action_space.__class__.__name__ == "Discrete":
            ...
            self.action_out = Dirichlet(inputs_dim, action_dim, use_orthogonal, gain)
    ...
    def forward(self, x, opt_actions, available_actions=None, deterministic=False):
        """
        Compute actions and action logprobs from given input.
        :param x: (torch.Tensor) input to network.
        :param opt_actions: (torch.Tensor) actions optimized by path-based method
        ...
        if self.mixed_action:
            ...
            self.act_time += 1
            if self.act_time % 100 == 0:
                print('self.act_time=', self.act_time)

            #actions = opt_actions

            if deterministic:
                actions = action_logit.mode()


            else:
                actions = action_logit.rsample()

            actions = torch.clamp(actions, min=1e-7)
